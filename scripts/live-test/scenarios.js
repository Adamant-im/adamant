...
/**
 * Runs a bounded no-delay transaction queue workload for one transaction type.
 * @param {object} context - Runner context.
 * @param {number} transactionType - Transaction type generated by the workload.
 */
async function runTxQueueStressScenario (context, transactionType) {
  const fixture = context.fixtureAccounts.transfer;
  ...
  async function worker () {
    while (Date.now() < deadline) {
      ...
      context.metrics.latency('txqueue.type' + transactionType + '.submit', result.latencyMs);
  ...
  const scenarioResult = {
    ...
    workload: {
      configuredDurationMs: TXQUEUE_DURATION_MS,
      actualDurationMs: workloadElapsedMs,
      concurrency: TXQUEUE_CONCURRENCY,
      artificialDelayMs: 0,
      generated,
      accepted,
      rejected,
  ...
  if (!scenarioResult.passed) {
    const error = Error(
        'Type ' +
        transactionType +
        ' transaction queue stress failed: generated ' +
        generated +
        ', transport failures ' +
        transportFailures +
        ', snapshot failures ' +
