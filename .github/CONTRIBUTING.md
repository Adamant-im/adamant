...
| `consensus.activation` | `consensus` | `testnet`, `localnet` | Verifies `fairSystem` and `spaceship` activation state and observable behavior on both `node-recipient` and `node-peer`, then checks cross-node chain agreement. |
...
Running `--suite consensus` is read-only and requires two accessible nodes. In testnet mode, `node-recipient` is the selected target and `node-peer` defaults to the third peer in `test/config.default.json`, matching the transaction load scenarios. In localnet mode, the first available node is `node-recipient` and the second is `node-peer`. The scenario requests node status, block status, the latest block, the active delegate list, a bounded transaction sample, and connected peers from both nodes.

The cross-node check always requires the same nethash and a height difference no greater than `--max-height-drift`. When both nodes are at the same height, it also requires the same broadhash, latest block id, ordered delegate public keys, and activation states. Broadhash, latest-block, and delegate-order equality are not asserted while heights differ because normal block propagation can temporarily make those values different.

The `fairSystem` activation replaces raw `vote` with productivity-adjusted `votesWeight` for delegate ranking, active-delegate selection, and approval calculation. Voter balances are divided across their selected delegates, then delegate weight is adjusted by productivity. Its goal is fairer voting influence while keeping delegate ordering deterministic. The live probe checks that the delegate list uses the ranking field selected for the observed height, applies the public-key tie-breaker, and reports approval as the selected weight divided by current supply.
