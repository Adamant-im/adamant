...
The transaction queue scenario captures `/api/node/status` and `/api/transactions/count` before generation, immediately after generation, 30 seconds after generation, and after every transaction accepted by the recipient node has been included in a block on every observation node. In testnet mode, the table contains `node-recipient`, which receives the submitted transactions, and `node-peer`, which defaults to the third peer in `test/config.default.json`. In localnet mode, every target node is observed.
